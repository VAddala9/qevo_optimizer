...
def fidelity(probs, M=1):                     
    return sum(ABCD(probs, i)[0] for i in range(M))/M
"""
Entropy:
    f = 0
    for i in range(M):
        ABCD_probs_i = np.array(ABCD(probs, i))
        f+=np.sum(ABCD_probs_i*np.log(ABCD_probs_i))
    return f
"""

def ABCD(probs, indices=0):
    indices = [indices] if isinstance(indices, int) else indices
    N = len(probs.shape)
    return np.sum(probs, axis=tuple(set(range(N))-set(indices))) # can replace with np.take

def entropy(probs, indices=0):
    # note this is the entropy of the system of qubits at indices, not the average marginal entropy
    marginal_probs = ABCD(probs, indices)
    return -np.sum(marginal_probs*np.log2(marginal_probs))

def average_entropy(probs, M):
    # average of the marginal entropies of each of the M Bell pairs
    return sum([entropy(probs, i) for i in range(M)])/M

def mutual_information(probs, x, y):
    x, y = min(x, y), max(x, y)
    P_xy = ABCD(probs, [x, y])
    P_x = ABCD(probs, x)
    P_y = ABCD(probs, y)
    return np.sum(np.nan_to_num(P_xy*np.log2(P_xy/(P_x.reshape(4, 1)@P_y.reshape(1, 4)))))

def average_mutual_information(probs, M):
    return sum(mutual_information(probs, *comb) for comb in itertools.combinations(range(M), 2))/(M*(M-1)/2)  if M > 1 else 0
    
def p_zero(probs, M):
    probs = ABCD(probs, range(M))
    return probs.flatten()[0]

def p_one(probs, M):
    probs = ABCD(probs, range(M))
    return sum([np.take(probs, indices=j, axis=i).flatten()[0] for j in [1, 2, 3] for i in range(M)])
...
class Individual:
    def __init__(self,ops,F,M=1,history=History.manual, weights=(1,0,0,0,0),qs=None): # qs is not particularly well tested
        self.ops = ops
        self._fitness = None
        self._fidelity_and_succ_prob = None
        self._average_mutual_information = None
        self._average_entropy = None
        self._p_zero = None
        self._p_one = None
        self.F = F
        self.M = M
    ...
    def __repr__(self):
        rep = 'Individual(M=%s, F=%s, history=History.%s, weights=\"%s\", ops=[\n           %s])'%(
            self.M, self.F, self.history.name, self.weights,
            ',\n           '.join(str(_) for _ in self.ops))
        return rep.replace("\"", "") if not isinstance(self.weights, str) else rep
    def __eq__(self, other):
    ...
    def copy(self, F=0.9, P2=0.99):
        return Individual([op.copy(F=F, P2=P2) for op in self.ops], F, self.M, self.history)
    ...
    def fidelity_and_succ_prob(self, reval=False):
        if reval or not self._fidelity_and_succ_prob:
            if not self.ops:
                ...
                self._average_mutual_information = 0
                self._average_entropy = -(self.F*np.log2(self.F) + (1-self.F)*np.log2((1-self.F)/3))
                self._p_zero = self.F**self.M 
                self._p_one = self.M*self.F**(self.M-1)*(1-self.F)/3
                return self.F, 1.
            p = 1.
            probs = get_new_probs(self.N, self.F, qs=self.qs)
            for o in self.ops:
                ...
                probs = o.apply(probs)
            self._probs = probs
            self._fidelity_and_succ_prob = fidelity(probs, self.M), p
            self._ABCD = ABCD(probs)
            self._average_mutual_information = average_mutual_information(probs, self.M)
            self._average_entropy = average_entropy(probs, self.M)
            self._p_zero = p_zero(probs, self.M)
            self._p_one = p_one(probs, self.M)
    ...
    def average_mutual_information(self):
        ...
        return self._average_mutual_information
    ...
    def probs(self):
        self.fidelity_and_succ_prob()
        return self._probs
    def total_bell_pairs(self):
        if not self.ops:
            return 0
        N = self.ops[0].N
        total = N
        last_ops_reg = set(range(N))
        for i,o in enumerate(self.ops[::-1]):
            if isinstance(o, Measurement):
                t = o.target
                if t in last_ops_reg:
                    last_ops_reg.remove(t)
                    if t < self.M:
                        total+=1
                else:
                    total+=1
    ...
    def fitness(self):
        if not self._fitness:
            ...
            elif self.weights == 'boostedI':
                return self.fidelity_and_succ_prob()[0] + self.average_mutual_information()
            elif self.weights == 'penalizedI':
                return self.fidelity_and_succ_prob()[0] - self.average_mutual_information()
            elif self.weights == 'boostedPzeroPone':
                return self.fidelity_and_succ_prob()[0] + self.p_zero() + self.p_one()
            elif self.weights == 'boostedWeightedPzeroPone':
                return self.fidelity_and_succ_prob()[0] + 0.1*(self.p_zero() + self.p_one())
            elif self.weights == 'PzeroPone':
                return self.p_zero() + self.p_one()
            elif self.weights == 'PzeroPonePenalizedI':
                return self.p_zero() + self.p_one() - self.average_mutual_information()
            elif self.weights == 'Pzero':
                return self.p_zero()
            elif self.weights == 'negativeI':
                return -self.average_mutual_information()
            elif self.weights == 'PzeroPonePenalizedS':
                return self.p_zero() + self.p_one() - self.average_entropy()
            elif self.weights == 'negativeS':
                return -self.average_entropy()
            elif self.weights == 'transmission_rate':
                return self.succ_prob()*(self.p_zero()+self.p_one())/self.total_bell_pairs()
            else:
    ...
    def generate_dataframe(self, F_start=0.7, Fs=np.linspace(0.7, 1, 16), P2s=[0.95, 0.99, 1], generated_by="genetic", save_to=None):
        ckts = []
        num_registers = []
        num_raw_bps = []
        num_purified_bps = []
        ...
        for F in Fs:
            for P2 in P2s:
                i = self.copy(F, P2)
                i.fidelity_and_succ_prob(True)
                ckts.append(repr(i))
                num_registers.append(N)
                num_raw_bps.append(i.total_bell_pairs())
                num_purified_bps.append(self.M)
                F_0s.append(F)
                avg_fidelities.append(i.fidelity())
                succ_probs.append(i.succ_prob())
                ps.append(P2)
                ckt_lens.append(L)
                gen_by.append(generated_by)
                circuit_type.append(self.history)
                circuit_hashes.append(circuit_hash)
                p_zero.append(i.p_zero())
                p_one.append(i.p_one())
                p_zero_one.append(i.p_zero()+i.p_one())
                mutual_information.append(i.average_mutual_information())
                entropy.append(i.average_entropy())
                weights.append(i.weights)
                
        data = {"Circuit": ckts, 
                "Registers": num_registers,
                "Raw Bell Pairs": num_raw_bps, 
                "Purified Pairs": num_purified_bps,
                "F_in": F_0s,
                "F_out": avg_fidelities,
                "Success Probability": succ_probs,
                "P2": ps,
                "Circuit Length": ckt_lens,
                "Generated by": gen_by,
                "Circuit Type": circuit_type,
                "Circuit Hash": circuit_hashes,
                "Mutual Information": mutual_information,
                "Average Entropy": entropy,
                "P_zero": p_zero,
                "P_one": p_one,
                "P_zero + P_one": p_zero_one,
                "Weights": weights,
                "Delft Fidelity": [p**(1/self.M) for p in p_zero],
                "Transmission Rate": [s*pzo/n for s, pzo, n in zip(succ_probs, p_zero_one, num_raw_bps)] # note this is not always applicable
        }
        
        df = pd.DataFrame(data)
        if save_to is not None:
            df.to_csv(f'{save_to}.csv')
        return df
    @classmethod
    def random(cls, ops_len, permitted_ops, N, F, P2, Mη, weights, M=1, qs=None):
        return cls([random.choice(permitted_ops).random(N, F, P2, Mη, qs=qs) for _ in range(ops_len)], F, M,
                   history=History.random, weights=weights, qs=qs)
    def new_drop_op(self):
        ops = self.ops.copy()
        del ops[random.randint(0,len(ops)-1)]
        return Individual(ops, self.F, self.M, history=History.drop_m, weights=self.weights)
    def new_gain_op(self, permitted_ops, N, F, P2, Mη):
        ...
        return Individual(ops, self.F, self.M, history=History.gain_m,
    ...
    def new_swap_op(self):
        ...
        ops[i] = self.ops[i+1]
        ops[i+1] = self.ops[i]
        return Individual(ops, self.F, self.M, history=History.swap_m,
                weights=self.weights, qs=self.qs)
    ...
    def new_child(self, p):
        ...
        return Individual(ops1[:i1]+ops2[i2:], self.F, self.M, history=History.child,
                weights=self.weights, qs=self.qs)
    ...
    def canonical(self):
        ...
        return reordered
    def raw_pairs(self): # valid only if purifying to 1 Bell pair
...
def individual2plot(ops,title='',fig_axis=None,shading=False,parallelism=False):
    ...
    for i,o in enumerate(ops[::-1]):
        if isinstance(o, Measurement):
            ...
            if t in last_ops_reg:
                last_ops_reg.remove(t)
                if t < M:
                    regeneration_indices.add(L-1-i)
            else:
                regeneration_indices.add(L-1-i)
        if isinstance(o, (CNOT, CNOTPerm)):
            for t in o.targets:
    ...
    for i,o in enumerate(ops):
        ...
        if isinstance(o,(CNOT, CNOTPerm)):
            ...
            if i in regeneration_indices:#i+backstep < L-N+1:
...
class Population:
    def __init__(self,
        ...
        self.MAX_RAW_PAIRS                   = MAX_RAW_PAIRS
    ...
    def _sort(self):
        self.l.sort(key=lambda _:(_.fitness()*max(_.total_bell_pairs()==self.MAX_RAW_PAIRS, (20-min(self.generations, 20))/20)), reverse=True)
...
#                self.generations, self.POPULATION_SIZE, self.l[0].fitness()-self.l[-1].fitness()), flush=True) 
    def run(self, min_delta=1e-15):
        ...
        try:
            while self.generations < self.MAX_GEN-1:
                ...
                self.report()
                if (self.l[0].fitness()-self.l[-1].fitness()<=min_delta) and self.generations > 10: 
    ...
    def generate_dataframe(self, F_start=0.7, Fs=np.linspace(0.7, 1, 16), P2s=[0.95, 0.99, 1], generated_by="genetic", save_to=None, num_individuals=1):
        df = pd.concat([indiv.generate_dataframe(F_start=F_start, Fs=Fs, P2s=P2s, generated_by=generated_by) for indiv in self.l[:num_individuals]], ignore_index=True)
